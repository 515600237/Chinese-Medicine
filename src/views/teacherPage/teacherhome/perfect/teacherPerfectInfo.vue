<template>
    <div class="wrapper">
        ...
        <div class="tabbar addRoleBox">
          ...
          <van-cell-group class="labelTab" v-if="userInfo.idCard.length != 18" >
            <van-row type="flex" justify="space-between" >
              <van-col span="12" class="text-left">性别</van-col>
              <van-col span="10" class="text-right">
                <van-tabs v-model="sexActive" type="card" style="margin-right：0;"
                          @click="tabClick">
          ...
          <van-cell-group>
            <van-field
              ...
              :is-link="userInfo.idCard.length != 18"
              placeholder="请选择"
              label="出生日期"
              readonly="readonly"
              input-align="right"
              :class ="{disabled:userInfo.idCard.length == 18}"
              @click="brithdayDate()"
            ></van-field>
...
<script>
    export default {
        ...
        methods: {
            ...
            /****点击保存提示****/
            isSaveAll() {
                /*判断是否是台湾身份证，如果是必须选择出生日期*/
                if(this.userInfo.idCard&&this.userInfo.idCard.length==10&&!this.userInfo.birthday){
                    this.$toast("请选择出生日期");
                    return;
                }
                ...
                this.utils.ajax({
                    ...
                    success: (res) => {
                        if (res.code == 0) {
                            ...
                            if(this.userInfo&&this.userInfo.sex){
